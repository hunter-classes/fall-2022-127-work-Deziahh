...
def fastMode(dataset):
    ...
    #loop thru the dataset and increment the buckets
    for item in dataset:
        tallies[item] = tallies[item] + 1
        
    #find the bucket with the largest value
    modeCount = findLargest(tallies)
    
    #find and return the index that holds modeCount in tallies
    for i in range(len(tallies)):
        if tallies[i] == modeCount:
            return i
